...
- 模型供应商列表不再返回真实 API Key，仅返回脱敏值。
- 本地上传增加文件名清洗、目录约束、流式写入和大小限制，阻止路径穿越与超大文件占用内存。
- 图片代理仅允许访问受控本地静态目录或公网 HTTP(S) 图片，阻止本地文件读取、SSRF、非图片响应和超大响应。
- 删除任务现在会清理任务结果、状态、转写、音频缓存、Agent trace、向量索引和数据库记录。
- 单容器镜像改为直接提供 Web 静态文件，不再反代到未启动的 8080 端口；Agent、截图和并发配置可透传到后端。
- Docker Compose 默认只绑定 `127.0.0.1`，避免无认证服务直接暴露到局域网。
- 浏览器扩展补全 `@vueuse/shared` 直接依赖和存储类型标注，恢复严格类型检查。
- 供应商测试与模型刷新会先保存当前表单；更新失败不再被静默吞掉。
- OpenAI 兼容地址会自动补齐根路径 `/v1`，并对 HTML/非 ChatCompletion 响应给出明确错误；同名模型现在按供应商与模型组合选择。
- Agent 工具选择现在有固定枚举和提示，并新增 `get_transcript` 高层工具，避免模型调用未注册工具导致任务失败。
- 后端启动时会自动发现并注入 FFmpeg 缓存路径，避免安装版下载音频后因 `WinError 2` 中断。
- 数据库初始化会显式注册供应商、模型和视频任务模型，确保 `video_tasks` 表在安装版中正常创建。
- 项目版本统一为 `0.2.8`。
