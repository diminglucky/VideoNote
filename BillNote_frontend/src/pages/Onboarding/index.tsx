...
const Onboarding = () => {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#f7f6f2] p-6">
      <div className="w-full max-w-2xl rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-center gap-3 border-b border-neutral-100 pb-5">
          <div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-lg border border-neutral-200 bg-white">
            <img src={logo} alt="VideoNote" className="h-full w-full object-contain" />
          </div>
          <div>
            <h1 className="text-xl font-semibold text-neutral-950">欢迎使用 VideoNote</h1>
            <p className="mt-1 text-sm text-neutral-500">完成基础配置后，就可以开始把视频整理成学习笔记。</p>
          </div>
        </div>
        ...
        <div className="mb-6 flex items-center gap-2 text-xs text-neutral-500">
          {[1, 2, 3, 4].map(s => (
            <div key={s} className="flex items-center gap-2">
              <div
                className={`flex h-7 w-7 items-center justify-center rounded-md border text-xs font-medium ${step >= s ? 'border-neutral-950 bg-neutral-950 text-white' : 'border-neutral-300 bg-white text-neutral-400'}`}
              >{s}</div>
              {s < 4 && <div className={`h-px w-10 ${step > s ? 'bg-neutral-950' : 'bg-neutral-300'}`} />}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <section className="flex flex-col gap-3">
            <h2 className="font-semibold text-neutral-950">第 1 步 · 后端连通性</h2>
            <p className="text-sm text-neutral-600">桌面端会自动启动 Python 后端进程。检查连通中…</p>
            {pinging && <div className="text-sm text-neutral-500">检测中…</div>}
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button className="rounded-md bg-neutral-950 px-4 py-1.5 text-sm text-white hover:bg-neutral-800 disabled:opacity-50" disabled={!backendOk} onClick={next}>
                下一步
              </button>
            </div>
          </section>
        ...
        {step === 2 && (
          <section className="flex flex-col gap-3">
            <h2 className="font-semibold text-neutral-950">第 2 步 · 模型供应商</h2>
            <p className="text-sm text-neutral-600">填一个 OpenAI 兼容供应商：DeepSeek / Qwen / Claude / 自托管 / OpenAI 都行。</p>
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-neutral-600">供应商名（自取）</span>
              <input className="rounded-md border border-neutral-300 px-2 py-1.5" value={providerName} onChange={e => setProviderName(e.target.value)} />
            </label>
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-neutral-600">API 地址</span>
              <input className="rounded-md border border-neutral-300 px-2 py-1.5" value={baseUrl} onChange={e => setBaseUrl(e.target.value)} />
            </label>
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-neutral-600">API Key</span>
              <input type="password" className="rounded-md border border-neutral-300 px-2 py-1.5" value={apiKey} onChange={e => setApiKey(e.target.value)} />
            </label>
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-neutral-600">模型名（如 gpt-4o-mini / deepseek-chat / qwen-turbo）</span>
              <input className="rounded-md border border-neutral-300 px-2 py-1.5" value={modelName} onChange={e => setModelName(e.target.value)} />
            </label>
            {error && <div className="text-xs text-red-600">{error}</div>}
            <div className="flex gap-2 justify-between">
              <button className="text-sm text-neutral-500 hover:text-neutral-800" onClick={prev}>上一步</button>
              <button className="rounded-md bg-neutral-950 px-4 py-1.5 text-sm text-white hover:bg-neutral-800 disabled:opacity-50" disabled={savingProvider} onClick={saveProvider}>
        ...
        {step === 3 && (
          <section className="flex flex-col gap-3">
            <h2 className="font-semibold text-neutral-950">第 3 步 · 音频转写引擎</h2>
            <p className="text-sm text-neutral-600">把视频音频转成文字。<strong>推荐在线引擎</strong>，避免本地下载 ~600MB 的模型。</p>
            <div className="grid gap-2">
              {[
                ...
                <label key={opt.value} className={`flex cursor-pointer gap-3 rounded-md border p-3 ${transcriberType === opt.value ? 'border-neutral-950 bg-neutral-50' : 'border-neutral-200 hover:border-neutral-300'}`}>
            ...
            <div className="flex gap-2 justify-between">
              <button className="text-sm text-neutral-500 hover:text-neutral-800" onClick={prev}>上一步</button>
              <button className="rounded-md bg-neutral-950 px-4 py-1.5 text-sm text-white hover:bg-neutral-800 disabled:opacity-50" disabled={savingTranscriber} onClick={saveTranscriber}>
        ...
        {step === 4 && (
          <section className="flex flex-col gap-3">
            <h2 className="font-semibold text-neutral-950">第 4 步 · Cookie 同步（可选）</h2>
            ...
            <div className="rounded-md border border-neutral-200 bg-neutral-50 p-3 text-xs text-neutral-600">
              提示：浏览器插件支持一键 cookie 同步；桌面版也可以在「下载配置」页手动复制。
            </div>
            <div className="flex gap-2 justify-between">
              <button className="text-sm text-neutral-500 hover:text-neutral-800" onClick={prev}>上一步</button>
              <button className="rounded-md bg-neutral-950 px-4 py-1.5 text-sm text-white hover:bg-neutral-800" onClick={finish}>
                完成，进入 VideoNote
              </button>
            </div>
          </section>
